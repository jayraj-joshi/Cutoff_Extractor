...
${minimal ? '' : `- Extract the "Home University" name for each college. Ensure the extracted "home_university" value is identical to the one found in the document (usually beside "Status").
...
[
  {
    "college_code": "string",
    "college_name": "string",
    ${minimal ? '' : '"home_university": "string",\n    "city": "string",\n    "status": "string",\n    "minority_status": "string",\n    "isMinority": "boolean",'}
    "branches": [
      {
        "branch_code": "string",
        "branch_name": "string",
...
- NO NULL VALUES: Cutoff category fields must NEVER be null. If a category is present in the document, extract its "rank" and "percentile" accurately as an object. If a category is not present for a specific stage, OMIT the field entirely from the JSON object instead of setting it to null.
${minimal ? '' : `- Extract the "Status" for each college (e.g., "Government", "Un-Aided", etc.).
