      ...
      <header className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-semibold tracking-tight text-slate-900 dark:text-white">MHT-CET Extractor</h1>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className="flex items-center gap-2 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-300 dark:disabled:bg-slate-800 text-white text-xs font-medium rounded-lg transition-colors shadow-sm"
            >
            ...
            <div className="flex items-center gap-4">
              <label className="flex items-center gap-2 cursor-pointer group">
                <div className="relative">
                  ...
                  />
                  <div className={`block w-8 h-5 rounded-full transition-colors ${minimal ? 'bg-indigo-600' : 'bg-slate-300 dark:bg-slate-700'}`}></div>
                  <div className={`absolute left-1 top-1 bg-white dark:bg-slate-200 w-3 h-3 rounded-full transition-transform ${minimal ? 'translate-x-3' : ''}`}></div>
                </div>
                <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider group-hover:text-slate-700 dark:group-hover:text-slate-200 transition-colors">Minimal</span>
              </label>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-3 flex flex-col gap-6">
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col h-[calc(100vh-10rem)]">
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <input 
                        className={`text-xs font-bold bg-transparent border-none focus:ring-0 p-0 w-full truncate ${
                          activeBatchId === batch.id ? 'text-indigo-900 dark:text-indigo-100' : 'text-slate-700 dark:text-slate-300'
                        }`}
                        value={batch.name}
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1.5">
                        <FileText className={`w-3 h-3 ${activeBatchId === batch.id ? 'text-indigo-500 dark:text-indigo-400' : 'text-slate-400 dark:text-slate-500'}`} />
                        <span className="text-[10px] text-slate-500 dark:text-slate-400 font-medium">{batch.files.length} PDFs</span>
                      </div>
          ...
          <div className="lg:col-span-5 flex flex-col gap-6">
              ...
              <div className="p-6">
                {activeTab === 'pdf' ? (
                  <div className="flex flex-col gap-4">
                    <div className="flex flex-wrap gap-3">
                      <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg text-sm font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors disabled:opacity-50"
                      >
                      ...
                      {activeBatch && activeBatch.files.length > 0 && (
                        <button
                          onClick={clearFiles}
                          disabled={activeBatch.isExtracting}
                          className="flex items-center gap-2 px-4 py-2 ml-auto text-sm font-medium text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors disabled:opacity-50"
                        >
                          Clear Batch
                        </button>
                      )}
                    ...
                    {activeBatch && activeBatch.files.length > 0 ? (
                      <div className="max-h-80 overflow-y-auto border border-slate-200 dark:border-slate-800 rounded-xl divide-y divide-slate-100 dark:divide-slate-800 bg-slate-50 dark:bg-slate-900/50">
                        {activeBatch.files.map(f => (
                          <React.Fragment key={f.id}>
                            <div className="p-3 flex items-center justify-between bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                            <div className="flex items-center gap-3 overflow-hidden">
                              {f.status === 'pending' && <Clock className="w-4 h-4 text-slate-400 dark:text-slate-500 shrink-0" />}
                              {f.status === 'processing' && <Loader2 className="w-4 h-4 text-indigo-500 dark:text-indigo-400 animate-spin shrink-0" />}
                              {f.status === 'success' && <CheckCircle2 className="w-4 h-4 text-emerald-500 dark:text-emerald-400 shrink-0" />}
                              {f.status === 'error' && <XCircle className="w-4 h-4 text-red-500 dark:text-red-400 shrink-0" />}
                              
                              <div className="truncate">
                                <p className="text-sm font-medium text-slate-700 dark:text-slate-200 truncate" title={f.file.name}>{f.file.name}</p>
                            ...
                            <div className="flex items-center gap-3 shrink-0 ml-4">
                              <button
                                ...
                                disabled={activeBatch.isExtracting}
                                className="p-1 text-slate-400 dark:text-slate-500 hover:text-red-500 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded transition-colors disabled:opacity-50"
                              >
                                <X className="w-4 h-4" />
                              </button>
                            </div>
                          ...
                          {f.status === 'error' && f.error && (
                            <div className="px-10 pb-3 -mt-1 bg-white dark:bg-slate-900">
                              <p className="text-[10px] text-red-400 dark:text-red-300 bg-red-50/50 dark:bg-red-900/10 p-1.5 rounded border border-red-100/50 dark:border-red-900/30 leading-tight">
                      ...
                      <div className="flex flex-col items-center justify-center border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-xl p-10 bg-slate-50 dark:bg-slate-900/50">
                        <Upload className="w-10 h-10 text-slate-400 dark:text-slate-600 mb-3" />
                        <p className="text-sm font-medium text-slate-900 dark:text-white">No PDFs in this batch</p>
                        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 text-center">
                          Add files to <span className="font-bold text-indigo-600 dark:text-indigo-400">{activeBatch?.name}</span> to start extracting.
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  <textarea
                    ...
                    className="w-full h-64 p-4 text-sm border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none resize-none font-mono"
                  />
                )}
              </div>
            </div>
            ...
            {activeBatch?.error && (
              <div className="flex items-start gap-3 p-4 bg-red-50 dark:bg-red-900/10 text-red-700 dark:text-red-400 rounded-xl border border-red-100 dark:border-red-900/30">
            ...
            <button
              ...
              className="w-full py-3.5 px-4 bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-300 dark:disabled:bg-slate-800 disabled:text-slate-500 dark:disabled:text-slate-600 disabled:cursor-not-allowed text-white text-sm font-medium rounded-xl shadow-sm transition-colors flex items-center justify-center gap-2"
            >
              ...
              <div className="flex items-center justify-between px-4 py-3 border-b border-slate-800 bg-slate-900/50">
                <div className="flex flex-col">
                  <h2 className="text-sm font-medium text-slate-200 flex items-center gap-2">
                    <span className={`w-2 h-2 rounded-full ${activeBatch?.result ? 'bg-emerald-500' : 'bg-slate-600 dark:bg-slate-700'}`}></span>
              ...
              <div className="flex-1 p-4 overflow-auto bg-[#0d1117]">
                {activeBatch?.result ? (
                  ...
                  <div className="h-full flex flex-col items-center justify-center text-slate-500 dark:text-slate-600 space-y-4">
                    <div className="w-16 h-16 rounded-full border-2 border-dashed border-slate-700 dark:border-slate-800 flex items-center justify-center">
                      <span className="text-2xl text-slate-600 dark:text-slate-700">{"{}"}</span>
                    </div>
